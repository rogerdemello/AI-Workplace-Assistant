...
def test_startup_accepts_a_real_secret_key():
    # Generated rather than written as a literal: a hardcoded high-entropy
    # string here is indistinguishable from a leaked credential to a secret
    # scanner, and this file should not be the reason one fires.
    import secrets

    from app.config import Settings, validate_security_settings

    validate_security_settings(Settings(SECRET_KEY=secrets.token_urlsafe(48)))
