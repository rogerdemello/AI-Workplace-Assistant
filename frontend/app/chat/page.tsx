...
import { PolicyResponse } from '@/components/chat/PolicyResponse'
...
export default function ChatPage() {
  ...
  const [showPolicyResponse, setShowPolicyResponse] = useState(false)
  ...
  const handleSendMessage = async (content: string) => {
    ...
    try {
      ...
      } else if (data.intent === 'ticket_create') {
        setShowTicketConfirmation(true)
        setCurrentTicketData(data.context?.ticket_data || {
          department: data.context?.department,
          issue: data.context?.issue,
          anonymous: data.context?.anonymous
        })
      } else if (data.intent === 'policy_query') {
        setShowPolicyResponse(true)
      }
    } catch (error) {
  ...
  const handleLeaveSubmit = async (leaveData: {
    ...
    const leaveMessage = `Leave request: ${leaveData.leave_type} from ${leaveData.start_date} to ${leaveData.end_date}. Reason: ${leaveData.reason}`
    setShowLeaveForm(false)
    await handleSendMessage(leaveMessage)
  }
    ...
    <div className="container mx-auto py-8">
      <Card>
        ...
        <CardContent>
          ...
          {showLeaveForm && (
            <div className="mt-4">
              <LeaveForm onSubmit={handleLeaveSubmit} onCancel={handleLeaveCancel} />
            </div>
          )}
          {showTicketConfirmation && (
            <div className="mt-4">
              <TicketConfirmation 
                department={currentTicketData.department}
                issue={currentTicketData.issue}
                anonymous={currentTicketData.anonymous}
              />
            </div>
          )}
          {showPolicyResponse && (
